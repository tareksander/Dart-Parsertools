...
String _ruleToString(Rule r, String method) {
  return "LexerRule(RegExp(${escapeDartString(r.pattern)}), $method, ${r.state})";
}

/// Creates a SharedPartBuilder for the lexer generator.
Builder lexerBuilder(BuilderOptions options) =>
    SharedPartBuilder([LexerGenerator()], "lexer");
  ...
  generateForAnnotatedElement(
    ...
    if (element is! ClassElement) {
      throw InvalidGenerationSourceError(
          "Lexer annotation is only valid for classes.");
    }
    ...
    for (var child in children) {
      ...
      if (rule != null) {
        rules.add((
          child,
          Rule(rule.getField(_patternField)!.toStringValue()!,
              rule.getField(_priorityField)!.toIntValue()!,
...

    {
      for (var r in rules) {
        if (r.$2.pattern.isEmpty) {
          throw InvalidGenerationSourceError(
              "Rule has empty pattern: ${r.$1.name}");
        }
      }
    }
    ...
    for (int k in priorities) {
      rulesString +=
          "[${ruleLists[k]!.map((e) => _ruleToString(e.$2, e.$1.name)).reduce((value, element) => "$value, $element")}],";
    }
    rulesString = "[$rulesString]";

    String classCode =
        "abstract class _$classname<T extends Token> extends LexerBase<T> {\n";
    for (var r in rules) {
      classCode +=
          "TokenResponse<T> ${r.$1.name}(String token, int line, int char, int index);\n";
    }
